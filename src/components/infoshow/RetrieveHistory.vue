      ...
      >
        <div class="h-full flex flex-col">
          <div class="p-4 flex justify-between items-center border-b" :class="isDarkMode ? 'border-gray-700' : 'border-gray-200'">
            <h2 class="text-2xl font-bold" :class="isDarkMode ? 'text-gray-200' : 'text-gray-800'">取件记录</h2>
            <button
              @click="closeHistory"
              ...
              <div class="flex justify-between items-center">
                <div class="flex-grow">
                  ...
                  <p class="text-sm mt-1" :class="isDarkMode ? 'text-gray-400' : 'text-gray-600'">
                    大小: {{ formatFileSize(upload.size) }}
                  </p>
                </div>
                <div class="flex space-x-2">
                  <button @click="viewDetails(upload)" :class="isDarkMode ? 'text-blue-400 hover:text-blue-300' : 'text-blue-500 hover:text-blue-600'" class="transition duration-300 ease-in-out">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
import FileRetrieveModal from './FileRetrieveModal.vue';
...
const uploads = fileDataStore.receiveData;
...
const handleDeleteUpload = (upload, index) => {
  fileDataStore.deleteReceiveData(index);
